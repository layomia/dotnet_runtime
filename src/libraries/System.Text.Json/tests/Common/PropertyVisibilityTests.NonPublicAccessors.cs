...
namespace System.Text.Json.Serialization.Tests
{
    ...
    {
        [Fact]
        public void NonPublic_AccessorsNotSupported_WithoutAttribute()
        {
            string json = @"{
                ""MyInt"":1,
                ""MyString"":""Hello"",
            ...
            var obj = JsonSerializerWrapper.Deserialize<MyClass_WithNonPublicAccessors>(json);
        ...
        [Fact]
        public void Honor_JsonSerializablePropertyAttribute_OnProperties()
        {
            string json = @"{
                ""MyInt"":1,
                ""MyString"":""Hello"",
            ...
            var obj = JsonSerializerWrapper.Deserialize<MyClass_WithNonPublicAccessors_WithPropertyAttributes>(json);
        ...
        [Fact]
        public void ExtensionDataCanHaveNonPublicSetter()
        {
            string json = @"{""Key"":""Value""}";

            // Baseline
            var obj1 = JsonSerializerWrapper.Deserialize<ClassWithExtensionData_NonPublicSetter>(json);
            Assert.Null(obj1.ExtensionData);
            Assert.Equal("{}", JsonSerializerWrapper.Serialize(obj1));

            // With attribute
            var obj2 = JsonSerializerWrapper.Deserialize<ClassWithExtensionData_NonPublicSetter_WithAttribute>(json);
            Assert.Equal("Value", obj2.ExtensionData["Key"].GetString());
            Assert.Equal(json, JsonSerializerWrapper.Serialize(obj2));
        }
        ...
        [Fact]
        public void HonorCustomConverter()
        {
            ...
            Assert.Throws<JsonException>(() => JsonSerializerWrapper.Deserialize<StructWithPropertiesWithConverter>(json));

            var obj = JsonSerializerWrapper.Deserialize<StructWithPropertiesWithConverter>(json, options);
            Assert.Equal(MySmallEnum.AnotherValue, obj.GetMyEnum);
            Assert.Equal(25, obj.MyInt);

            // ConverterForInt32 throws this exception.
            Assert.Throws<NotImplementedException>(() => JsonSerializerWrapper.Serialize(obj, options));
        }
        ...
        [Fact]
        public void HonorCaseInsensitivity()
        {
            var options = new JsonSerializerOptions { PropertyNameCaseInsensitive = true };

            string json = @"{""MYSTRING"":""Hello""}";
            Assert.Null(JsonSerializerWrapper.Deserialize<MyStruct_WithNonPublicAccessors_WithTypeAttribute>(json).MyString);
            Assert.Equal("Hello", JsonSerializerWrapper.Deserialize<MyStruct_WithNonPublicAccessors_WithTypeAttribute>(json, options).MyString);
        }
        ...
        [Fact]
        public void HonorJsonPropertyName()
        {
            string json = @"{""prop1"":1,""prop2"":2}";

            var obj = JsonSerializerWrapper.Deserialize<StructWithPropertiesWithJsonPropertyName>(json);
            Assert.Equal(MySmallEnum.AnotherValue, obj.GetMyEnum);
        ...
        [Fact]
        public void Public_And_NonPublicPropertyAccessors_PropertyAttributes()
        {
            string json = @"{""W"":1,""X"":2,""Y"":3,""Z"":4}";

            var obj = JsonSerializerWrapper.Deserialize<ClassWithMixedPropertyAccessors_PropertyAttributes>(json);
        ...
        public void NonPublicProperty_WithJsonInclude_Invalid(Type type)
        {
            ...
            ex = Assert.Throws<InvalidOperationException>(() => JsonSerializerWrapper.Serialize(Activator.CreateInstance(type), type));
            exAsStr = ex.ToString();
            Assert.Contains("MyString", exAsStr);
            Assert.Contains(type.ToString(), exAsStr);
            Assert.Contains("JsonIncludeAttribute", exAsStr);
