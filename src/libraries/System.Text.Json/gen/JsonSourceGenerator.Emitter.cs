            ...
            private void GenerateTypeInfo(TypeGenerationSpec typeGenerationSpec)
            {
                ...
                switch (typeGenerationSpec.ClassType)
                {
                    ...
                    case ClassType.Object:
                        {
                            source = typeGenerationSpec.ConstructionStrategy == ObjectConstructionStrategy.ParameterizedConstructor
                                ? GenerateForObjectWithParamCtor(typeGenerationSpec)
                                : GenerateForObject(typeGenerationSpec);
...
");
    ...
    return {PropVarName};
            ...
            private string GenerateCtorParamMetadataInitFunc(TypeGenerationSpec typeGenerationSpec, string ctorParamInitMethodName)
            {
                ...
                ParameterInfo[] parameters = typeGenerationSpec.CtorParameterInfoArray;
...
");
    ...
    {WriterVarName}.WriteEndObject();");
            ...
            private static string GetParameterizedCtorInvocationFunc(TypeGenerationSpec typeGenerationSpec)
            {
                ...
                {
                    sb.Append($"{GetParamUnboxing(parameters[i].ParameterType, i)}, ");
                }

                sb.Append($"{GetParamUnboxing(parameters[lastIndex].ParameterType, lastIndex)})");

                return sb.ToString();

                static string GetParamUnboxing(Type type, int index) => $"({type.GetCompilableName()}){ArgsVarName}[{index}]";
            }
