...
namespace System.Text.Json.SourceGeneration
{
    public sealed partial class JsonSourceGenerator
    {
        private sealed class Parser
        {
            ...
            /// </summary>
            private readonly Dictionary<Type, TypeGenerationSpec> _typeMetadataCache = new();
            ...
            public SourceGenerationSpec? GetGenerationSpec(List<ClassDeclarationSyntax> classDeclarationSyntaxList)
            {
                ...
                if (jsonSerializableAttributeSymbol == null || jsonSerializerContextSymbol == null)
                {
                    return null;
                }
                ...
                foreach (ClassDeclarationSyntax classDeclarationSyntax in classDeclarationSyntaxList)
                {
                    ...
                    if (!IsSubtypeOfJsonSerializerContext(classDeclarationSyntax, jsonSerializerContextSymbol, compilationSemanticModel))
                    {
                        continue;
                    }
                    ...
                    foreach (AttributeListSyntax attributeListSyntax in classDeclarationSyntax.AttributeLists)
                    {
                        ...
                        if (jsonSerializableAttributeSymbol.Equals(attributeSymbol.ContainingType, SymbolEqualityComparer.Default))
                        {
                    ...
                    {
                        SerializerOptions = serializerOptions ?? new JsonSerializerOptionsAttribute(),
            ...
            private bool IsSubtypeOfJsonSerializerContext(
            ...
            private TypeGenerationSpec? GetRootSerializableType(SemanticModel compilationSemanticModel, AttributeSyntax attributeSyntax)
            {
                ...
                foreach (AttributeArgumentSyntax node in attributeArguments)
                {
                    ...
                    // TODO: also fetch generation mode and pass it to GetOrAddTypeMetadata below.
                ...
                return GetOrAddTypeGenerationSpec(type, typeInfoPropertyName);
            }

            private TypeGenerationSpec GetOrAddTypeGenerationSpec(Type type, string? typeInfoPropertyName = null, JsonSourceGenerationMode generationMode = default)
            {
                if (_typeMetadataCache.TryGetValue(type, out TypeGenerationSpec? typeMetadata))
                {
                    return typeMetadata!;
                }
                ...
                typeMetadata.Initialize(
                    ...
                    typeInfoPropertyName: typeInfoPropertyName ?? type.GetFriendlyTypeName(),
                    generationMode,
                    type,
                    classType,
            ...
            private PropertyGenerationSpec GetPropertyGenerationSpec(MemberInfo memberInfo)
            {
                ...
                return new PropertyGenerationSpec
                {
                    ...
                    DeclaringTypeRef = memberInfo.DeclaringType.GetUniqueCompilableTypeName(),
